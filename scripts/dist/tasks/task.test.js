        ...
        const p = new Parser((results) => {
            ...
            if (results.failures) {
                results.failures.forEach((fall) => {
                    trace.error(project.dir + ':' + f, ' | line:' + fall.diag.at.lineNumber, fall.diag.at.columnNumber);
...
export const coverageTest = () => new Promise(async (done) => {
    const t = Date.now();
...
export async function testProjects(projects) {
    const b = bench();
