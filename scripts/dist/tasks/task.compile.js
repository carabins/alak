...
export async function compile(project) {
    ...
    if (declarationsMix) {
        ...
        fs.readdirSync(declarationsPath).forEach((f) => {
            ...
            fs.copyFileSync(path.resolve(declarationsPath, f), path.join(artTypesPath, f));
        });
        const refs = [];
    ...
    const fixEsmImports = (code, packageDir, packageName) => {
        // 1. Replace internal package aliases (e.g., 'alak/namespaces' -> './namespaces.mjs')
        code = code.replace(new RegExp(`(['"])${packageDir}/([^'"]+)(['"])`, 'g'), "$1./$2.mjs$3");
        ...
        code = code.replace(selfImportRegex, "$1./$2.mjs$3");
        // 3. Replace other @alaq package imports to include .mjs extension
        code = code.replace(/(['"])(@alaq\/[^'"]+)(['"])/g, (match, quote1, importPath, quote2) => {
        ...
        code = code.replace(/(['"])(\.\.?\/[^'"]+)(['"])/g, (match, quote1, importPath, quote2) => {
    ...
    Object.keys(srcFiles).forEach((srcFile) => {
        ...
        if (!src.isDeclaration) {
            ...
            const esmCodeFixed = fixEsmImports(esmCode, project.dir, project.packageJson.name);
...
//# sourceMappingURL=task.compile.js.map