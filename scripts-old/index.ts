...
import { buildTask, getProjectChoices, getTaskChoices, startTask, xTask, rolldownTask } from '~/scripts/tasks'
...
async function start() {
  ...
  switch (process.argv[2]) {
    ...
    case 'rolldown':
      return await startTask(rolldownTask, allProjects)
    case 'cover':
      return coverageTest()
    case 'test':
    ...
    case 'x':
      return await startTask(xTask, allProjects)
  }
  if (!selectedTask) {
