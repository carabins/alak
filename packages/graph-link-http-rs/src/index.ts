...
export function generate(ir: IR, options: GenerateOptions = {}) {
  ...
  for (const ns of ir.namespaces) {
    const dirName = ns.name.replace(/\./g, '_');
    files.push({
      path: `${dirName}/mod.rs`,
      content: generateMod(ns, { header, rtCrate }),
    });
  }
...
function generateMod(ns: Namespace, opts: { header: boolean; rtCrate: string }) {
  const lines: string[] = [];
  ...
  // Client Struct
  const rawName = ns.name.split('.').pop() || 'Client';
  ...
  for (const a of ns.actions) {
    const outputType = a.output ? mapRustType(a.output, true) : '()';
