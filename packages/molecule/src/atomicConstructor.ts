...
export function atomicConstructor<M, E, N, Events extends readonly string[]>(
  ...
  const getFromNode = ([nodeKey, targerKey]: string[]) => {
    ...
    !node && console.error(`empty sub-node [${nodeKey + "." + targerKey}] in atomic node ${name.toUpperCase()} `)
  ...
  constructor.edges && constructor.edges.forEach(e => {
    const listiners = []
    ...
    if (typeof e.from === 'string') {
      ...
      if (!strategyMethod) {
        console.error(`unsupported strategy [ ${e.strategy.toUpperCase()}  ]in atomic node ${name.toString()} for edge`, e)
