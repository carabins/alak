...
export function setAtomValue(atom: Core, value, context?) {
  const setValue = (finalValue) => {
    ...
    notifyChildes(atom)
    if (atom.isStateless) delete atom.value
    return finalValue
  }
  if (value && value.then) {
...
export function notifyChildes(atom: Core) {
  const v = atom.value
  const apply = atom.isHoly ? (f) => f(...v) : (f) => (f.length == 2 ? f(v, atom._) : f(v))
...
export const createCore = (...a) => {
  const atom = function (...v) {
    ...
    if (v.length) {
      const value = atom.isHoly ? v : v[0]
      if (debug.enabled)
        return setAtomValue(atom, value, !atom.isHoly && v[1] ? v[1] : AtomContext.direct)
