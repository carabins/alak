...
**Default dev endpoint.** If you omit `endpoint`/`token`, the plugin assumes a locally-running Logi from `A:/source/logi/docker-compose.yml` at `http://localhost:2025` with project token `demo_project_token`. To run one:
...
logiPlugin()   // → localhost:2025, demo_project_token, release auto-tagged
```
...
Includes an **integration test** (`test/integration.test.ts`) that sends real frames to `http://localhost:2025/ingest/v1/json`. Auto-skips if Logi is not reachable — run `docker compose up -d` in `A:/source/logi` first to enable it.
