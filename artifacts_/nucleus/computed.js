...
function from(...fromNucleons) {
    ...
    function applyValue(mixedValue) {
        if ((0, _utils.isPromise)(mixedValue)) {
            mixedValue.then((v)=>{
                freeWaiters(v);
                (0, _quark.setNucleonValue)(quark, v);
            });
        } else {
            freeWaiters(mixedValue);
            (0, _quark.setNucleonValue)(quark, mixedValue);
        }
        quark.isAwaiting && delete quark.isAwaiting;
        return mixedValue;
    }
    const makeMix = (mixFn)=>{
        ...
        const values = fromNucleons.map((a)=>{
            if (a.isAwaiting) {
                inAwaiting.push(a);
            } else if (needFull && !(0, _utils.alive)(a.value)) {
                inAwaiting.push(a);
            }
            return a.value;
        });
                ...
                if ((0, _utils.isPromise)(v)) {
                    ...
                    v.then((v)=>{
                        ...
                        if (!isWaiting()) {
                            ...
                            if (!(0, _utils.isPromise)(deepValue)) {
        ...
        function mixer(v, a) {
            ...
            if (!finite || v !== linkedValue) {
                ...
                if (rune) {
                    ...
                    if ((0, _utils.isPromise)(args)) args.then(quark);
                    else quark(args);
                }
            }
        }
