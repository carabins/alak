...
## Package Creation & Maintenance (CRITICAL)

**READ**: `PACKAGE_CREATION_GUIDE.md` for complete instructions.

### The Fundamental Rule

**Source package.json (packages/*/) must NOT contain:**
- ❌ `"exports"` — Auto-generated from src/ structure
- ❌ `"license"` — Generated by build
- ❌ `"author"` — Generated by build
- ❌ `"repository"` — Generated by build
- ❌ `"files"` — Generated by build
- ❌ `"peerDependencies"` — Generated by build
- ❌ `"main"` — Generated by build
- ❌ `"types"` — Generated by build

If any of these appear in source package.json = **DIRTY** and needs cleanup.

### When You Find Dirty package.json

1. Report immediately with list of forbidden fields
2. Show exact clean version (with only: name, version, description, type, scripts, dependencies)
3. Ask user before fixing
4. Explain: Build script auto-detects exports from directory structure, keeps single source of truth

### Exports Defined by Directory Structure

```
src/index.ts → "."
src/presets/nucleus.ts → "./nucleus"
src/plugins/array.ts → "./plugins/array"
src/index.node.ts, src/index.browser.ts → platform-specific exports
```

Build script sees structure and generates complete exports map in `artifacts/*/package.json`.

### Consistency Checks

When reviewing any package:
- [ ] Source package.json contains only: name, version, description, type, scripts, dependencies
- [ ] No forbidden fields in source
- [ ] src/presets/ and src/plugins/ directories exist (if exports specify them)
- [ ] Platform-specific files match conditional exports
